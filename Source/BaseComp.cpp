...
{
    // set languge..
    LocalisedStrings::setCurrentMappings(new LocalisedStrings(s, true));
	...
	langBt->setToggleState(true, dontSendNotification);
...
{    
    if (comboBox == toneSetCb) // set tone..
    {
        ...
        case 1:
			...
			fileInfoLb->setText(TRANS("Sine Wave"), dontSendNotification);
            break;

        case 2:
            samplerSource->setSampledSound(SamplerSource::PIANO);
            fileInfoLb->setText(TRANS("Grand Piano"), dontSendNotification);
            break;

        case 3:
            samplerSource->setSampledSound(SamplerSource::VIOLA);
            fileInfoLb->setText(TRANS("Solo Viola"), dontSendNotification);
            break;

        case 4:
            samplerSource->setSampledSound(SamplerSource::TAIKO);
            fileInfoLb->setText(TRANS("Drum TaiKo"), dontSendNotification);
            break;

        default:
            fileInfoLb->setText(String::empty, dontSendNotification);
            break;
        }
    }
...
void BaseComp::buttonClicked(Button* button)
{
    if (button == setupBt)  // show setup audio device..
    {
        if (aboutBt->getToggleState())
        {
            aboutBt->setToggleState(false, dontSendNotification);
        }
    ...
    else if (button == aboutBt)  // show about info..
    {
        ...
        {
            setupBt->setToggleState(false, dontSendNotification);
            selector->setVisible(false);
        }
...
{
    sltLb->setText(TRANS("Preset: "), dontSendNotification);
    rootLb->setText(TRANS("Root: "), dontSendNotification);
